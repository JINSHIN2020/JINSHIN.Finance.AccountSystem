...
{
    public partial class FrmPayTicketMgt : Form
    {
        public List<JINSHINFINANCE063> forms { get; set; }
        public int cnt { get; set; }
        public int curridx { get; set; }
        public string papernum { get; set; }
        public InvModel invmodel { get; set; }
        public PayTicketPaperVModel vmodel { get; set; }
        public int supplyno { get; set; }

        public FrmPayTicketMgt()
        {
            InitializeComponent();
            forms = new List<JINSHINFINANCE063>();
            cnt = 20;
            supplyno = DataClass.SupplyNo.Value;
            invmodel = new InvModel();
            vmodel = new PayTicketPaperVModel(dgData, cnt);
        ...
        {
            using (FrmPayTicketPaper addform = new FrmPayTicketPaper())
            {
                addform.ShowDialog();
        ...
        private void btnMUpdate_Click(object sender, EventArgs e)
        {
            if (dgData.CurrentRow != null)
            {
                ...
                using (FrmPayTicketUpdate updateform = new FrmPayTicketUpdate())
                {
        ...
        {
            vmodel.CreatePayAccTicketData(dgData);
            forms = vmodel.SearchData(true, forms, txtNo.Text, txtUser.Text, txtCompany.Text,
                                        supplyno, lblNoData01, lblmoney, dgData);
        }
        ...
        {
            papernum = "";
            curridx = -1; forms = vmodel.SearchData(true, forms, txtNo.Text, txtUser.Text, txtCompany.Text, supplyno, lblNoData01, lblmoney, dgData);
            if (dgData.CurrentRow != null)
            {
        ...
        private void FrmPayTicketMgt_Shown(object sender, EventArgs e)
        {
            forms = vmodel.SearchData(true, forms, txtNo.Text, txtUser.Text, txtCompany.Text, supplyno, lblNoData01, lblmoney, dgData);
