...
namespace JINSHIN.Finance.AccountSystem.FrmControl.Business.Accounts
{
    public partial class FrmRecPaper : Form
    {
        ...
        private void FrmRecPaper_Shown(object sender, EventArgs e)
        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!CheckNumberControl())
                    return;
                JINSHINFINANCE061 model = new JINSHINFINANCE061();
                ...
                model.PersonCreate = DataClass.empName;
                using (TableFrmNo tableno = new TableFrmNo())
                {
                    model.PaperNum =  tableno.GetTableNo(new Interface.Data.Services.File.CTATableDataNo() { Num=3, TableInt=3, 
                        SupplyNo=supplyno });
                }
                using (AccountEntityDB content = new AccountEntityDB())
                {
                    ...
                    {
                        entity.JINSHINFINANCE061.Add(model);
                        entity.SaveChanges();
                    }
                }
            }
